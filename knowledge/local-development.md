...
For runtime testing, place the plugin where a Noctalia development source can discover it, enable `tmelik/system-summary`, and add `tmelik/system-summary:summary` to a bar. Confirm all seven values render, unavailable sensors show `—`, the tooltip is readable, and left-click opens Control Center's System tab.
...
4. run the upstream community repository checks after placing this directory at `system-summary/`;
