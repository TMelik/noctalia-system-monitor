...
description: Implementation and verification state for System Monitor 1.5.0.
...
The source is published at `TMelik/noctalia-system-monitor`. The prior community-store package was submitted from branch `TMelik:add-system-monitor` in [noctalia-dev/community-plugins#460](https://github.com/noctalia-dev/community-plugins/pull/460). Version 1.5.0 passes repository validation and has been loaded through the local override for graphical tooltip and bar-capsule verification; publication still requires an explicit commit and push.
